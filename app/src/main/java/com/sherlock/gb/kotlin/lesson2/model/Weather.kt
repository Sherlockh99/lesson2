...
/**
 * Обратите внимание, как мы создаём список: listOf. Создание аналога ArrayList в Kotlin с
 * возможностью читать только данные списка, но не добавлять в него новые.
 * Это так называемый immutable (неизменяемый список).
 * Если понадобится создать изменяемый список, используем стандартный метод mutableListOf
 */
